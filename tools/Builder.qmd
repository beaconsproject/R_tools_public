...
2. Set the folder path to the benchmarkbuilder executable and the unzipped BEACONs_R_Tools folder. 
```r
...
nghbrs <- neighbours(catchments)
reserve_seed <- reserve_seeds(catchments, CAs_sf=conservation_areas, CAs_name = "reserve", areatarget_value = 10000, joinType = "INTERSECT")
