...
1. Download the zip folder containing BEACONs R Tools functions, data and example 
- <a href="../downloads/BEACONs_R_Tools.zip" download>Download BEACONs R Tools</a>

2. Unzip BEACONS_R_Tools.zip, create the folder structure and point on the folder containing the benchmarkbuilder executable
```r
# Path to the ZIP file
zip_path <- "path/to/BEACONs_R_Tools.zip"
...
# Unzip
unzip(zipfile = zip_path, exdir = dest_folder)
...
builder(catchments_sf = catchments, seeds = seed, neighbours = nghbrs, out_dir = file.path(dest_folder, "Builder_output"), builder_local_path = dest_folder)
```
