...
- conservation_area_id: Column in \code{conservation_area_sf} specify unique identifier. Default is network.
...
if(addDCI){
  ...
  mutate(dci = sapply(1:nrow(reserves_sf), function(i) {
    i <- reserves_sf[i, ]
    calc_dci(i, streams_sf, conservation_area_id = colName) 
  }))
}
if(addLWDCI){
  reserves_sf <- reserves_sf %>%
  mutate(lwdci = sapply(1:nrow(reserves_sf), function(j) {
    j <- reserves_sf[j, ]
    calc_lwdci(j, streams_sf, conservation_area_id = colName) 
  }))
}
