...
#'
#' @param conservation_area_sf sf object of protected area polygons.
#' @param conservation_area_id String matching the unique identifier column in \code{conservation_area_sf}.
...
get_upstream_catchments <- function(conservation_area_sf, conservation_area_id, catchments_sf){
  ...
  catchments_sf <- make_all_integer(catchments_sf, cols = "CATCHNUM")
  
  # get list of catchnums in each PA
  CAs_catchnums_tab <- catchnums_in_polygon(conservation_area_sf, conservation_area_id, catchments_sf)
...
get_downstream_catchments <- function(conservation_area_sf, conservation_area_id, catchments_sf){
...
#' @param conservation_area_id character specify unique identifier. Default is network.
...
calc_lwdci <- function(conservation_area_sf, stream_sf, conservation_area_id = "network", buffer_width = 0.1){
  ...
  reserve_lwdci <- sf::st_intersection(conservation_area_sf, stream_prepped) %>% # intersect with reserves to get buffered stream for each reserve
    dplyr::select(dplyr::all_of(conservation_area_id), BASIN) %>%
    sf::st_cast("MULTIPOLYGON", warn = FALSE) %>% # this is needed to avoid geometries being lost in the POLYGON cast
    sf::st_cast("POLYGON", warn = FALSE) %>% # explode into individual stream segments
    dplyr::mutate(stream_length = as.numeric(sf::st_area(geometry)) / buffer_width) %>% # divide area by buffer to get length of each stream segment
    sf::st_drop_geometry() %>% # drop the geometry for speed
    dplyr::group_by(!!sym(conservation_area_id), BASIN) %>% # for each network and basin...
  ...
  # reserves that do not intersect the stream network get dropped during st_intersection.
  # join dci back to original reserves and set missing reserves to have dci of 0
  lwdci <- dplyr::left_join(sf::st_drop_geometry(conservation_area_sf), reserve_lwdci, by = conservation_area_id) %>%
